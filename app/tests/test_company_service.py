...
async def test_create_company(ac: AsyncClient, uow: IUnitOfWork, current_user_id: int):
    company_create = CompanyCreate(name="Test Company", description="A test company")
...
async def test_update_company(ac: AsyncClient, uow: IUnitOfWork, current_user_id: int):
    ...
    created_company = await company_service.create_company(
        uow, company_create, current_user_id
    )

    company_update = CompanyUpdate(
        name="Updated Company", description="An updated test company"
    )
    updated_company = await company_service.update_company(
        uow, created_company.id, company_update, current_user_id
    )
...
async def test_get_companies(ac: AsyncClient, uow: IUnitOfWork, current_user_id: int):
    ...
    for i in range(15):
        company_create = CompanyCreate(
            name=f"Test Company {i}", description=f"A test company {i}"
        )
        await company_service.create_company(uow, company_create, current_user_id)

    companies_response = await company_service.get_companies(
        uow, skip=0, limit=10, request_url="http://test.com/companies"
    )
...
async def test_get_company_by_id(
    ac: AsyncClient, uow: IUnitOfWork, current_user_id: int
):
    company_service = CompanyService()
