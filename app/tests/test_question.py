...
async def test_create_question(uow: IUnitOfWork, current_user_id: int):
    ...
    # Create a question
    question_response = await service.create_question(uow, 1, question_data, current_user_id)
...
async def test_get_questions_by_quiz_id(uow: IUnitOfWork, current_user_id: int):
    ...
    questions_response = await service.get_questions_by_quiz_id(uow, quiz_id, current_user_id, skip, limit)

    assert len(questions_response) > 0


async def test_permission_denied_create_question(uow: IUnitOfWork, current_user_id: int):
    question_data = QuestionSchemaCreate(
        quiz_id=1,
        question_text="Question 1",
        answers=[
